...
function setLogLevel(l: Level) {
  level = l;
}
function setTraceFlags(flags: string[]) {
...
function log(l: Level, message: string, meta?: Record<string, unknown>) {
  if (
    !shouldLog(
      l,
      meta && typeof meta.flag === "string" ? String(meta.flag) : undefined,
    )
  )
    return;
  const entry = {
...
class Metrics {
  ...
  snapshot() {
    ...
    for (const [k, arr] of this.hist) {
      ...
      const pct = (p: number) =>
        sorted[Math.min(n - 1, Math.floor(p * (n - 1)))];
      durations[k] = {
        count: n,
        p50: pct(0.5) || 0,
        p95: pct(0.95) || 0,
